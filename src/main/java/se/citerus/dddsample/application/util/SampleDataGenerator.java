...
import static se.citerus.dddsample.domain.model.voyage.SampleVoyages.DALLAS_TO_HELSINKI;
import static se.citerus.dddsample.domain.model.voyage.SampleVoyages.DALLAS_TO_HELSINKI_ALT;
import static se.citerus.dddsample.domain.model.voyage.SampleVoyages.HELSINKI_TO_HONGKONG;
import static se.citerus.dddsample.domain.model.voyage.SampleVoyages.HONGKONG_TO_NEW_YORK;
...
public class SampleDataGenerator {
	...
	public SampleDataGenerator(PlatformTransactionManager transactionManager, SessionFactory sf,
			CargoRepository cargoRepository, VoyageRepository voyageRepository, LocationRepository locationRepository,
	...
	private static void loadHandlingEventData(JdbcTemplate jdbcTemplate) {
		String handlingEventSql = "insert into HandlingEvent (completionTime, registrationTime, type, location_id, voyage_id, cargo_id) "
				+ "values (?, ?, ?, ?, ?, ?)";

		Object[][] handlingEventArgs = {
				// XYZ (SESTO-FIHEL-DEHAM-CNHKG-JPTOK-AUMEL)
				{ ts(0), ts((0)), "RECEIVE", 1, null, 1 }, { ts((4)), ts((5)), "LOAD", 1, 1, 1 },
				{ ts((14)), ts((14)), "UNLOAD", 5, 1, 1 }, { ts((15)), ts((15)), "LOAD", 5, 1, 1 },
				{ ts((30)), ts((30)), "UNLOAD", 6, 1, 1 }, { ts((33)), ts((33)), "LOAD", 6, 1, 1 },
				{ ts((34)), ts((34)), "UNLOAD", 3, 1, 1 }, { ts((60)), ts((60)), "LOAD", 3, 1, 1 },
				{ ts((70)), ts((71)), "UNLOAD", 4, 1, 1 }, { ts((75)), ts((75)), "LOAD", 4, 1, 1 },
				{ ts((88)), ts((88)), "UNLOAD", 2, 1, 1 }, { ts((100)), ts((102)), "CLAIM", 2, null, 1 },

				// ZYX (AUMEL - USCHI - DEHAM -)
				{ ts((200)), ts((201)), "RECEIVE", 2, null, 3 }, { ts((202)), ts((202)), "LOAD", 2, 2, 3 },
				{ ts((208)), ts((208)), "UNLOAD", 7, 2, 3 }, { ts((212)), ts((212)), "LOAD", 7, 2, 3 },
				{ ts((230)), ts((230)), "UNLOAD", 6, 2, 3 }, { ts((235)), ts((235)), "LOAD", 6, 2, 3 },

				// ABC
				{ ts((20)), ts((21)), "CLAIM", 2, null, 2 },

				// CBA
				{ ts((0)), ts((1)), "RECEIVE", 2, null, 4 }, { ts((10)), ts((11)), "LOAD", 2, 2, 4 },
				{ ts((20)), ts((21)), "UNLOAD", 7, 2, 4 },

				// FGH
				{ ts(100), ts(160), "RECEIVE", 3, null, 5 }, { ts(150), ts(110), "LOAD", 3, 3, 5 },

				// JKL
				{ ts(200), ts(220), "RECEIVE", 6, null, 6 }, { ts(300), ts(330), "LOAD", 6, 3, 6 },
	...
	private static void loadCarrierMovementData(JdbcTemplate jdbcTemplate) {
		String voyageSql = "insert into Voyage (id, voyage_number) values (?, ?)";
		Object[][] voyageArgs = { { 1, "0101" }, { 2, "0202" }, { 3, "0303" } };
		executeUpdate(jdbcTemplate, voyageSql, voyageArgs);

		String carrierMovementSql = "insert into CarrierMovement (id, voyage_id, departure_location_id, arrival_location_id, departure_time, arrival_time, cm_index) "
				+ "values (?,?,?,?,?,?,?)";

		Object[][] carrierMovementArgs = {
				// SESTO - FIHEL - DEHAM - CNHKG - JPTOK - AUMEL (voyage 0101)
				{ 1, 1, 1, 5, ts(1), ts(2), 0 }, { 2, 1, 5, 6, ts(1), ts(2), 1 }, { 3, 1, 6, 3, ts(1), ts(2), 2 },
				{ 4, 1, 3, 4, ts(1), ts(2), 3 }, { 5, 1, 4, 2, ts(1), ts(2), 4 },

				// AUMEL - USCHI - DEHAM - SESTO - FIHEL (voyage 0202)
				{ 7, 2, 2, 7, ts(1), ts(2), 0 }, { 8, 2, 7, 6, ts(1), ts(2), 1 }, { 9, 2, 6, 1, ts(1), ts(2), 2 },
				{ 6, 2, 1, 5, ts(1), ts(2), 3 },

				// CNHKG - AUMEL - FIHEL - DEHAM - SESTO - USCHI - JPTKO (voyage 0303)
				{ 10, 3, 3, 2, ts(1), ts(2), 0 }, { 11, 3, 2, 5, ts(1), ts(2), 1 }, { 12, 3, 6, 1, ts(1), ts(2), 2 },
				{ 13, 3, 1, 7, ts(1), ts(2), 3 }, { 14, 3, 7, 4, ts(1), ts(2), 4 } };
	...
	private static void loadCargoData(JdbcTemplate jdbcTemplate) {
		String cargoSql = "insert into Cargo (id, tracking_id, origin_id, spec_origin_id, spec_destination_id, spec_arrival_deadline, transport_status, current_voyage_id, last_known_location_id, is_misdirected, routing_status, calculated_at, unloaded_at_dest) "
				+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		Object[][] cargoArgs = { { 1, "XYZ", 1, 1, 2, ts(10), "IN_PORT", null, 1, false, "ROUTED", ts(100), false },
				...
				{ 5, "FGH", 1, 3, 5, ts(50), "IN_PORT", null, 1, false, "ROUTED", ts(100), false }, // Cargo origin
																									// differs from spec
																									// origin
	...
	private static void loadLocationData(JdbcTemplate jdbcTemplate) {
		...
		Object[][] locationArgs = { { 1, "SESTO", "Stockholm" }, { 2, "AUMEL", "Melbourne" },
				{ 3, "CNHKG", "Hongkong" }, { 4, "JPTOK", "Tokyo" }, { 5, "FIHEL", "Helsinki" },
				{ 6, "DEHAM", "Hamburg" }, { 7, "USCHI", "Chicago" } };
		executeUpdate(jdbcTemplate, locationSql, locationArgs);
	}

	private static void loadItineraryData(JdbcTemplate jdbcTemplate) {
		String legSql = "insert into Leg (id, cargo_id, voyage_id, load_location_id, unload_location_id, load_time, unload_time, leg_index) "
				+ "values (?,?,?,?,?,?,?,?)";

		Object[][] legArgs = {
				// Cargo 5: Hongkong - Melbourne - Stockholm - Helsinki
				{ 1, 5, 1, 3, 2, ts(1), ts(2), 0 }, { 2, 5, 1, 2, 1, ts(3), ts(4), 1 },
				{ 3, 5, 1, 1, 5, ts(4), ts(5), 2 },
				// Cargo 6: Hamburg - Stockholm - Chicago - Tokyo
				{ 4, 6, 2, 6, 1, ts(1), ts(2), 0 }, { 5, 6, 2, 1, 7, ts(3), ts(4), 1 },
	...
	public static void loadHibernateData(TransactionTemplate tt, final SessionFactory sf,
			final HandlingEventFactory handlingEventFactory, final HandlingEventRepository handlingEventRepository) {
		System.out.println("***加载 Hibernate数据 ***");
		tt.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
				...
				try {
					HandlingEvent event1 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-01"),
							trackingId, null, HONGKONG.unLocode(), HandlingEvent.Type.RECEIVE);
					session.save(event1);

					HandlingEvent event2 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-02"),
							trackingId, HONGKONG_TO_NEW_YORK.voyageNumber(), HONGKONG.unLocode(),
							HandlingEvent.Type.LOAD);
					session.save(event2);

					HandlingEvent event3 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-05"),
				...
				RouteSpecification routeSpecification1 = new RouteSpecification(HANGZOU, STOCKHOLM,
						toDate("2009-03-18"));
				...
				try {
					HandlingEvent event1 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-01"),
							trackingId1, null, HANGZOU.unLocode(), HandlingEvent.Type.RECEIVE);
					session.save(event1);

					HandlingEvent event2 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-03"),
							trackingId1, HONGKONG_TO_NEW_YORK.voyageNumber(), HANGZOU.unLocode(),
							HandlingEvent.Type.LOAD);
					session.save(event2);
					...
					HandlingEvent event4 = handlingEventFactory.createHandlingEvent(new Date(), toDate("2009-03-06"),
							trackingId1, HONGKONG_TO_NEW_YORK.voyageNumber(), NEWYORK.unLocode(),
							HandlingEvent.Type.LOAD);
					session.save(event4);
