...
public class GraphTraversalServiceImpl implements GraphTraversalService {
	...
	public List<TransitPath> findShortestPath(final String originNode, final String destinationNode,
		...
		for (int i = 0; i < candidateCount; i++) {
			...
			transitEdges.add(new TransitEdge(dao.getTransitEdge(originNode, firstLegTo), originNode, firstLegTo,
					fromDate, toDate));
			...
			fromDate = nextDate(date);
			toDate = nextDate(fromDate);
			transitEdges.add(new TransitEdge(dao.getTransitEdge(lastLegFrom, destinationNode), lastLegFrom,
					destinationNode, fromDate, toDate));
